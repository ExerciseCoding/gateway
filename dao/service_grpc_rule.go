...
type GrpcRule struct {
	...
	HeaderTransfot string `json:"header_transfor" gorm:"column:header_transfor" description:"header转换支持增加(add)、删除(del)、修改(edit)、格式: add headname headvalue"`
}
...
func (t *GrpcRule) ListBYServiceID(c *gin.Context, tx *gorm.DB, serviceID int64) ([]GrpcRule, int64, error) {
	...
	query = query.Where("sevice_id=?", serviceID)
